function DownloadNMAP {
    ...
    Remove-Item -Force -Path "C:\temp\nmap.zip"
}

function cleanAllNMAPData {
...
function RunWebReport {
    getLocalSubnets | foreach {
    $network = $_ -replace '/','-'
    $networkr = [system.String]::Join(" ", $($_.split(".")[0,1,2])) -replace ' ','.'
    $directs = gci -Path C:\temp\nmap\nmap-7.70\ -Recurse -Filter "Direct-*"
    $directs = $directs.Name
    $directs | Sort-Object | foreach {
        $direct = [system.String]::Join(" ", $($_.split(".")[0,1,2])) -replace ' ','.'
        $directp = $_
            if($direct -match "${networkr}$") {
                $fpath = "C:\temp\nmap\nmap-7.70\$($directp)"
                Get-Content -Path $fpath | foreach {
                    if($_ -match "^[80]|^[443]|^[\|]|^[N]+[m]") {
                        Write-Host $_ *>> C:\temp\nmap\nmap-7.70\Report-$network.txt
                    }
                }
            }
        }
...
function mapLocalSubnetsWebServers {
    getLocalSubnets | foreach {
        ...
        & nmapTcpSubnetPortOpen -port 80 -ip $_ *>> C:\temp\nmap\nmap-7.70\Local-$network-$starttime-T80.txt
        $starttime = Get-Date -Uformat "%H-%M-%S-%d-%m-%y"
        & nmapTcpSubnetPortOpen -port 443 -ip $_ *>> C:\temp\nmap\nmap-7.70\Local-$network-$starttime-T443.txt
    }
    $locals = gci -Path C:\temp\nmap\nmap-7.70\ -Recurse -Filter "*Local-*"
    $locals = $locals.Name
    $locals | foreach {
